...
const DigitalMarketing = () => {
    ...
    useEffect(() => {
        const load = async () => {
            const data = await portfolioStore.getByCategory('digital-marketing');
            setProjects(data || []);
            setLoading(false);
        };
        load();
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {currentProjects.map((item) => (
                                <Link 
                                    ...
                                    className="group relative h-96 bg-dark-card rounded-2xl overflow-hidden border border-white/5 hover:border-green-500/50 transition-all block"
                                >
                                    ...
                                    <div className="absolute inset-0 flex flex-col justify-end p-6 z-20">
                                        <div className="w-10 h-10 bg-green-500/20 backdrop-blur-md rounded-xl mb-4 flex items-center justify-center text-green-400 text-lg group-hover:scale-110 transition-transform">
                                            📱
                                        </div>
                                        <h3 className="text-white font-bold text-xl mb-2 group-hover:text-green-400 transition-colors">{item.title}</h3>
